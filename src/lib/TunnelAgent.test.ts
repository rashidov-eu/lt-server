...
describe('TunnelAgent', function() {
  it('should create an empty agent', async function() {
    const agent = new TunnelAgent();
    assert.equal(agent.isStarted(), false);
  ...
  it('should create a new server and accept connections', async function() {
    const agent = new TunnelAgent();
    assert.equal(agent.isStarted(), false);
  ...
  it('should reject connections over the max', async function() {
    const agent = new TunnelAgent({
      maxTcpSockets: 2,
    });
    assert.equal(agent.isStarted(), false);
  ...
  it('should queue createConnection requests', async function() {
    const agent = new TunnelAgent();
    assert.equal(agent.isStarted(), false);
  ...
  it('should should emit a online event when a socket connects', async function() {
    const agent = new TunnelAgent();
    const info = await agent.listen();
  ...
  it('should emit offline event when socket disconnects', async function() {
    const agent = new TunnelAgent();
    const info = await agent.listen();
  ...
  it('should emit offline event only when last socket disconnects', async function() {
    const agent = new TunnelAgent();
    const info = await agent.listen();
  ...
  it('should error an http request', async function() {
